...
#![warn(
    ...
    reason = "If something unsafe is made safe, there should be no confusion whether it is actually safe"
)]
#![forbid(
    clippy::cmp_null,
...
#![deny(
    ...
    reason = r#"3D is extremely delicate and requires extra caution. This lints are permitted, but must be documented when used with #[allow(..., reason = "...")]."#
)]
...
use crate::core::{
    color::Color,
    math::{
        BoundingBox, Matrix, Transform, {Vector2, Vector3, Vector4},
    },
    texture::Image,
...
// NOTE: RL_SUPPORT_MESH_GPU_SKINNING is enabled by default and raylib-rs does not disable it at the time of writing

/// Maximum number of maps supported
#[allow(
    clippy::as_conversions,
    reason = "MAX_MATERIAL_MAPS is 12 by default, which will never overflow or truncate when cast to usize"
)]
const MAX_MATERIAL_MAPS: usize = consts::MAX_MATERIAL_MAPS as usize;
...
#[derive(Debug)]
#[repr(transparent)]
pub struct WeakMesh<'src>(ffi::Mesh, PhantomData<&'src Mesh>);

impl Clone for WeakMesh<'_> {
    #[inline]
    fn clone(&self) -> Self {
        *self
    }
}
impl Copy for WeakMesh<'_> {}
...
impl Mesh {
    /// Convert a [`ffi::Mesh`] to a [`Mesh`].
    ///
    /// Returns [`None`] if any `..._count` field is negative, `vao_id` is zero, or `vbo_id` is null.
    ///
    /// # Safety
    ///
...
#[derive(Debug)]
#[repr(transparent)]
pub struct WeakMaterial<'src>(ffi::Material, PhantomData<&'src Material>);

impl Clone for WeakMaterial<'_> {
    #[inline]
    fn clone(&self) -> Self {
        *self
    }
}
impl Copy for WeakMaterial<'_> {}

/// Material, includes shader and maps
#[derive(Debug)]
pub struct Material {
    /// Material shader
    pub shader: ffi::Shader,
    /// Material maps array (MAX_MATERIAL_MAPS)
    maps: NonNull<[ffi::MaterialMap; MAX_MATERIAL_MAPS]>,
...
impl BoneInfo {
    ...
    pub const fn name(&self) -> &str {
        // SAFETY: TBD: BoneInfo name must be nul-terminated
        let cstr = unsafe { CStr::from_ptr(self.name.as_ptr()) };
        // SAFETY: name is always compatible with str
        unsafe { str::from_utf8_unchecked(cstr.to_bytes()) }
    }

    #[inline]
    pub const fn set_name(&mut self, name: &str) {
        let (front, [nul, ..]) = self.name.split_at_mut(name.len()) else {
            panic!("too many characters in name; not enough room for nul")
        };
        // SAFETY: ref to ref is guaranteed non-null
        front.copy_from_slice(unsafe { &*(name.as_bytes() as *const [u8] as *const [c_char]) });
        #[allow(
            clippy::cast_possible_wrap,
            reason = "u8 and c_char are guaranteed compatible"
        )]
        {
            *nul = b'\0' as c_char;
        }
...
#[derive(Debug)]
#[repr(transparent)]
pub struct WeakModel<'src>(ffi::Model, PhantomData<&'src Model>);

impl Clone for WeakModel<'_> {
    #[inline]
    fn clone(&self) -> Self {
        *self
    }
}
impl Copy for WeakModel<'_> {}

/// Model, meshes, materials and animation data
#[derive(Debug)]
pub struct Model {
    /// Local transform matrix
    pub transform: ffi::Matrix,
    ...
    material_count: u31,
    /// Meshes array
    meshes: NonNull<ffi::Mesh>,
    /// Materials array
    materials: NonNull<ffi::Material>,
    /// Mesh material number
    mesh_material: NonNull<i32>,

    // Animation data
    /// Number of bones
    bone_count: u31,
    /// Bones information (skeleton)
    bones: NonNull<ffi::BoneInfo>,
...
#[derive(Debug)]
#[repr(transparent)]
pub struct WeakModelAnimation<'src>(ffi::ModelAnimation, PhantomData<&'src ModelAnimation>);

impl Clone for WeakModelAnimation<'_> {
    #[inline]
    fn clone(&self) -> Self {
        *self
    }
}
impl Copy for WeakModelAnimation<'_> {}
...
pub struct ModelAnimation {
    ...
    frame_poses: NonNull<*mut ffi::Transform>,
    /// Animation name
    name: [c_char; 32],
}
...
// const _: () = {
//     assert!(
//         std::mem::offset_of!(MeshVboId, position)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_POSITION as usize * size_of::<u32>()
//     );
//     assert!(
//         std::mem::offset_of!(MeshVboId, texcoord)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD as usize * size_of::<u32>()
//     );
//     assert!(
//         std::mem::offset_of!(MeshVboId, normal)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL as usize * size_of::<u32>()
//     );
//     assert!(
//         std::mem::offset_of!(MeshVboId, color)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR as usize * size_of::<u32>()
//     );
//     assert!(
//         std::mem::offset_of!(MeshVboId, tangent)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_TANGENT as usize * size_of::<u32>()
//     );
//     assert!(
//         std::mem::offset_of!(MeshVboId, texcoord2)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD2 as usize * size_of::<u32>()
//     );
//     assert!(
//         std::mem::offset_of!(MeshVboId, indices)
//             == ffi::RL_DEFAULT_SHADER_ATTRIB_LOCATION_INDICES as usize * size_of::<u32>()
//     );
...
// const _: () = {
//     assert!(
//         std::mem::offset_of!(Mesh, vertex_count) == std::mem::offset_of!(ffi::Mesh, vertexCount)
//     );
//     assert!(
//         std::mem::offset_of!(Mesh, triangle_count)
//             == std::mem::offset_of!(ffi::Mesh, triangleCount)
//     );
//     assert!(std::mem::offset_of!(Mesh, vertices) == std::mem::offset_of!(ffi::Mesh, vertices));
//     assert!(std::mem::offset_of!(Mesh, texcoords) == std::mem::offset_of!(ffi::Mesh, texcoords));
//     assert!(std::mem::offset_of!(Mesh, texcoords2) == std::mem::offset_of!(ffi::Mesh, texcoords2));
...
//     assert!(std::mem::offset_of!(Mesh, indices) == std::mem::offset_of!(ffi::Mesh, indices));
//     assert!(
//         std::mem::offset_of!(Mesh, anim_vertices) == std::mem::offset_of!(ffi::Mesh, animVertices)
//     );
//     assert!(
//         std::mem::offset_of!(Mesh, anim_normals) == std::mem::offset_of!(ffi::Mesh, animNormals)
//     );
//     assert!(std::mem::offset_of!(Mesh, bone_ids) == std::mem::offset_of!(ffi::Mesh, boneIds));
//     assert!(
//         std::mem::offset_of!(Mesh, bone_weights) == std::mem::offset_of!(ffi::Mesh, boneWeights)
//     );
//     assert!(
//         std::mem::offset_of!(Mesh, bone_matrices) == std::mem::offset_of!(ffi::Mesh, boneMatrices)
//     );
//     assert!(std::mem::offset_of!(Mesh, bone_count) == std::mem::offset_of!(ffi::Mesh, boneCount));
//     assert!(std::mem::offset_of!(Mesh, vao_id) == std::mem::offset_of!(ffi::Mesh, vaoId));
//     assert!(std::mem::offset_of!(Mesh, vbo_id) == std::mem::offset_of!(ffi::Mesh, vboId));
